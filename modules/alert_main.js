...
module.exports = function(msg) {
	...
	if (major_version <= 5) {		// Old API. Providing a callback makes the window not block the process.
		...
		electron.dialog.showMessageBox({message: utils.stringify(msg), title: "Alert", buttons: ["OK"]}, () => {
			alerts_open--;
		});

	} else {						// New promise-based API. Despite what older (pre-v12) docs say, I don't think this ever blocks.

		electron.dialog.showMessageBox({message: utils.stringify(msg), title: "Alert", buttons: ["OK"]}).then(() => {
			alerts_open--;
		});
