...
# To build Optics: (for info see https://electronjs.org/docs/tutorial/application-distribution)
#
...
for key, value in zips.items():
	if not os.path.exists(value):
		continue
	build_dir = "dist/optics-{}-{}".format(version, key)
	...
	if os.path.exists(os.path.join(build_dir, "electron.exe")):
		os.rename(os.path.join(build_dir, "electron.exe"), os.path.join(build_dir, "optics.exe"))
	if os.path.exists(os.path.join(build_dir, "electron")):
		os.rename(os.path.join(build_dir, "electron"), os.path.join(build_dir, "optics"))
